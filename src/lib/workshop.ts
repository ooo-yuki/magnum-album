...
export const WORKSHOP_COST = 200;
const APP_PORT = 3000;
 ...
 */
export async function deployAndStartRun(sandboxId: string, prompt: string, isEdit: boolean): Promise<void> {
  const sandbox = await getStartedSandbox(sandboxId);
  const input = JSON.stringify({ prompt, apiKey: requireZenKey(), isEdit });
