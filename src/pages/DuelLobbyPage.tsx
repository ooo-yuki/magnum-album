...
export function DuelLobbyPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (src === "duel_rush" && score) {
        setDuelShareHint(`🌋 Duel Rush: твой скор ${score} → дуэль 42с · код ${c ? c.toUpperCase() : "ABCD"} — жми Создать ABCD`);
      }
    } catch {}
  }, []);

  // autocreate when ?create=1
  useEffect(() => {
    if (autoCreateDone || !connected || !me) return;
    try {
      const sp = new URLSearchParams(window.location.search);
      if (sp.get("create") === "1" || sp.get("autocreate") === "1") {
        const wagerParam = Number(sp.get("wager") ?? 42) as Wager;
        const w: Wager = ([0,42,142,420] as const).includes(wagerParam as any) ? wagerParam : 42;
        if (wsRef.current?.readyState === WebSocket.OPEN) {
          wsRef.current.send(JSON.stringify({ type: "lobby:create", wager: w }));
          setAutoCreateDone(true);
          showToast(`Автодуэль создана — ждём братух 🌋`);
        }
      }
    } catch {}
  }, [connected, me, autoCreateDone, showToast]);
