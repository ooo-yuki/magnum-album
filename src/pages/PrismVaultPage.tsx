...
export function PrismVaultPage() {
  ...
  const dismantle = useCallback(async (id: string) => {
    try {
      ...
      const d = await r.json() as { dust?: number; reward?: number; error?: string };
      ...
      toast(`Разобрано +${d.reward} пыли · dust ${d.dust}`);
  ...
  const craftAurora = useCallback(async (id: string) => {
    ...
    if (!item) return;
    // aurora epic → legendary uses dust; others use coins 3×common craft
    const isDustCraft = item.rarity === "epic" || item.rarity === "legendary";
    if (isDustCraft && dust < item.price) { toast(`Нужно ${item.price} пыли, у тебя ${dust}`, "err"); return; }
    try {
      // legendary/epic aurora craft via dust prism endpoint
      if (isDustCraft) {
        ...
        const d = await r.json() as { dust?: number; error?: string; crafted?: string };
    ...
    <div ref={rootRef} style={{ maxWidth: 1100, margin: "0 auto", padding: "18px 14px 40px" }}>
      ...
      <div data-aurora-hero style={{ borderRadius: 18, padding: 18, marginBottom: 16, background: "linear-gradient(135deg,rgba(0,255,204,0.12),rgba(88,101,242,0.10) 45%,rgba(255,68,204,0.08))", border: "1px solid rgba(0,255,204,0.22)", boxShadow: "0 0 28px rgba(0,255,204,0.14)" }}>
        ...
        <p style={{ margin: 0, opacity: 0.72, fontSize: 13, lineHeight: 1.45 }}>
          12 скинов neon prism + aurora epic 1420 · 42/142/420/1420 common→legendary · крафт 42 · разбор +100 dust · epic конус aurora + spin 3s
          <br />kemerovo-neon 142 · meduza-aurora 420 · gold-aurora-spin epic 1420 spin 3s · magnum_cosmetics + magnum_shop_inventory + magnum-forge-dust
        </p>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 10, fontSize: 12, fontWeight: 800 }}>
          ...
          <span style={{ padding: "6px 12px", borderRadius: 999, background: "rgba(255,204,0,0.10)", border: "1px solid rgba(255,204,0,0.25)", color: "#ffcc00" }}>✨ {dust} пыли</span>
      ...
      <div ref={gridRef} data-forge-reveal style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0,1fr))", gap: 12 }}>
        {NEON_PRISM_CATALOG.map((co: typeof NEON_PRISM_CATALOG[number]) => {
          ...
          const canDust = dust >= co.price;
          const canCoins = coins >= co.price;
          ...
          return (
            ...
            >
              ...
              <div style={{ opacity: 0.62, fontSize: 11, margin: "4px 0 8px" }}>
                {co.slot} · 🪙 {co.price} {isEpic ? "· aurora epic 420" : isLegend ? "· epic 1420" : co.rarity === "rare" ? "· kemerovo-neon 142" : "· common 42"} · пыль {co.price}
              </div>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {isOwned ? (
                  ...
                  <button
                    ...
                    style={{ width: "100%", padding: "8px 10px", borderRadius: 10, fontWeight: 800, border: "1px solid rgba(0,255,204,0.35)", background: canDust ? "linear-gradient(90deg,#00ffcc,#5865f2)" : "rgba(255,255,255,0.06)", color: canDust ? "#0a1a14" : "#fff", cursor: "pointer", opacity: canDust ? 1 : 0.85 }}
                  >
                    ✨ {co.price} пыли {co.rarity === "legendary" ? "· gold-aurora-spin 1420" : co.id.includes("meduza") ? "· meduza-aurora 420" : ""}
                  </button>
                ) : (
                  <button
                    type="button"
                    onClick={() => craftAurora(co.id)}
                    style={{ width: "100%", padding: "8px 10px", borderRadius: 10, fontWeight: 800, border: "1px solid rgba(255,255,255,0.12)", background: canCoins ? "linear-gradient(90deg,#ffcc00,#ff9d1e)" : "rgba(255,255,255,0.06)", color: canCoins ? "#1a1a0a" : "#fff", cursor: "pointer" }}
                  >
      ...
      <p style={{ marginTop: 12, fontSize: 12, opacity: 0.62 }}>
        12 aurora: kemerovo-neon 142 · meduza-aurora 420 · gold-aurora-spin epic 1420 spin 3s · 42/142/420/1420 common→legendary · крафт 42 (3×common) · разбор +100 dust (epic) · GSAP stagger y20 0.08 · aurora epic forge spring
      </p>
