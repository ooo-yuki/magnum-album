...
## [0.3.19] — 2026-09-01 📚 Data 5 фактов 01.09 + Россия42 + SLAY 70/30 — 1/10

> **2 коммита** `ba8ff51` → `49f7d48` · **+66 / −0** · **2 файла** · research §19 (0 новых клипов ФУГА TV за 3ч + Россия42 23.05.2025 Слава КПСС×5opka 324k + SLAY интервью «до сентября расписано» + 6/6 VTT 414K) + §20 (5 фактов 01.09.2026 — Кузбасс 66,8k/ДТП КАМАЗ vs автобус/ложные минирования/SLAY 70/30 WINLINE/бан 06.04) + `reports/data-2026-09-01-2256.md` 38L · рейтинг **1/10** (66 строк/30м, норма 5000+ = 10/10) · `tsc 0` · data-only без кода

### 📚 Контент — §19 + §20: 10 фактов, 6 VTT, 0 выдумок

- 📖 **§19 — YouTube ФУГА TV 0 новых за 3ч + Россия42 + SLAY цитата (data 22:07)** — `ba8ff51` **ФУГА 0 новых** топ-5 без изменений vs 18:59 `«С 1 сентября!» 01.09 168k / ТУСА МЕДУЗА 26.08 228k / BBQ 17.07 119k / МЕГА РУМ-ТУР 14.07 287k / VPN 27.04 363k` `yt-dlp --dump-json --playlist-items 1-5` 424 видео, **Россия42** `077q4azWvNk` 23.05.2025 Слава КПСС×5opka 2:40 324k 13k 👍 `music.thefence.me/425opkakpss` + цитата VTT/the-flow `«Ровно дышишь как на йоге…» / «красота и уродство… сырников Юли Высоцкой»` + **SLAY** `respawn.media` `«ощущение что я тут хозяин… всё расписано аж до сентября»` 3 статуэтки → **«Россия42» = 42 в названии страны** связывает Кузбасс-код 42 (§17 п.4) с масштабом страны, пробел VPN→XXL закрыт, **6/6 VTT** 414K `/tmp/yt42/*.vtt` transcript:true — `research.md` +28 §19, `reports/yt-2026-09-01-2207.md` локально
- 📖 **§20 — 5 новых фактов 01.09.2026 (data 22:56)** — `ba8ff51`+`49f7d48` **1)** День знаний Кузбасс 01.09.26 66,8k Кемерово 5 300 первоклашек мэр Анисимов шк.№10/лицей №89 ремонт 750млн + шк.36 281 в 10 классах `kem.kp.ru/online/news/7150485 + ngs42.ru/education/2026/09/01/76616542` → Recaps 01.09/About42 **2)** ЧП 31.08-01.09 ложные минирования кардиодиспансер + Подгорбунского Островского эвакуации `kuzbass.aif.ru + mk-kuzbass.ru 11:36` **3)** ДТП 01.09 14:35 Суворова Ленинск-Кузнецкий КАМАЗ vs автобус 18 пасс. госпитализация `ria.ru/20260901/dtp-2114622142` **4)** SLAY 2025 70/30 (зрители 13-27.11 сайт + жюри) Дворец Ирины Винер 03.12 WINLINE статуэтки Minecraft-стример впервые 5opka `gamemag.ru/news/196845 + esports.ru + championat.com` **5)** Бан 06.04.26 «сексуальный контент» `«забанили — завтра клип снимаем. эпичнее»` `taverna.gg` — `research.md` +28 §20, `reports/data-2026-09-01-2256.md` +38
- ✅ **Методика — 0 выдумок, все URL кликабельны, VTT верифицированы** — `ba8ff51` web_search 5 запросов + yt-dlp 5+6 видео + ФУГА топ-5 → 6 VTT 77+19+18+262+16+22=414K auto-ru + шумные куски The Flow сверены дословно, `49f7d48` web_search 5×5=25 результатов → 5 непокрытых факта §11-19 + источники kp/ngs42/aif/mk/ria/gamemag/esports/championat/taverna — честно без выдумок

### 🐛 Фиксы — нет, data-only

- 🩹 **Без кода — только данные, tsc 0 сохраняется** — 2 коммита не трогали `server.ts/src` — dirty 60+ файлов остаются carry → 0.3.20 (60 M + 90 ??)

### 📖 Дока + Ревью — 1/10 (падение 4/10 → 1/10, data-only)

- 👁️ **Review 1/10 — 66 строк/30м (data-only)** — **1/10** (0-499 = 1/10) — падение **4/10 (1010) → 1/10**, **2 коммита/30м** (0 продуктивных кода + 2 data) `+66/−0=66` (продуктивных кода `0` → **0/10**), шкала `0-49=0 … 5000+=10` — `tsc 0` · `gallery 9 файлов` · `health 200` · data-scout без выдумок держится — carry 60 M + 90 ?? → 0.3.20

---

## [0.3.18] — 2026-09-01 🔒 Gacha TX + Unique + Spin42 + Pass/Flashmob — 4/10

> **1 коммит** `27c2801` → `51d2f05` · **+1010 / −129** · **5 файлов** · Gacha `BEGIN/FOR UPDATE/COMMIT/ROLLBACK` закрывает double-spend гонку + `ON CONFLICT` dedup · `0022` `magnum_idea_votes/bookmarks (user,idea) UNIQUE` · `0023` `magnum_board_shares (user,day) UNIQUE` + `magnum_gacha_history` + `magnum_gacha_free_rolls PK` + `magnum_flashmob_*` 3 таблицы · Spin Wheel 42 8 секторов + реферальный QR + стрик ×2/×3 + Pass XP hooks · рейтинг **4/10** (1010 строк, норма 5000+ = 10/10) · review 1928 #1 закрыт

### 🔒 Безопасность — Gacha double-spend/race закрыт (P0 #1 review 1928)

- 🔒 **Gacha Roll TX — `BEGIN → SELECT … FOR UPDATE → UPDATE → COMMIT / ROLLBACK`** — `51d2f05` **до:** `SELECT balance → если хватает → UPDATE -price` без транзакции — параллельные `POST /magnum/api/gacha/roll` списывали баланс дважды (double-spend, TOCTOU) — **после:** `await sql BEGIN` + `SELECT balance FROM magnum_coins WHERE user_id FOR UPDATE` + `SELECT pity … FOR UPDATE` + всё `INSERT magnum_pity/gacha_history/transactions` внутри TX + `COMMIT` в конце + `catch → ROLLBACK → 500 db error` — гонка закрыта, баланс консистентен — `server.ts` `handleGachaRoll` +129/−87, `handleGachaFreeRoll` аналогично `INSERT … ON CONFLICT DO NOTHING RETURNING → 0 rows ⇒ 429 already` + `FOR UPDATE` + `BEGIN/COMMIT` — `server.ts` +1059/−129
- 🛡️ **Gacha Free Roll dedup — `ON CONFLICT DO NOTHING` + `RETURNING`** — `51d2f05` `magnum_gacha_free_rolls (user_id, day_id) PK` — `SELECT dup → INSERT` → гонка `2× free roll` → теперь атомарно `INSERT … ON CONFLICT DO NOTHING RETURNING user_id → 0 rows ⇒ ROLLBACK 429`, без `SELECT` перед `INSERT` — `server.ts` `handleGachaFreeRoll` +23/−12
- 🔒 **Board share dedup — `UNIQUE (user_id, day_id)`** — `51d2f05` `drizzle 0023` `CREATE UNIQUE INDEX magnum_board_shares_user_day_unique` + `drizzle/schema.ts` `uniqueIndex` на `magnumBoardShares` — дубли `POST /magnum/api/board/share` теперь `ON CONFLICT` без двойной выдачи — `drizzle/migrations/0023_gacha_board_tx.sql` +1, `drizzle/schema.ts` +1

### 🗄️ Neon — 0022 + 0023 + Flashmob 3 таблицы

- 🗄️ **0022 — `magnum_idea_votes/bookmarks UNIQUE (user,idea)` — ON CONFLICT dedup** — `51d2f05` `drizzle/migrations/0022_idea_votes_unique.sql` +4 `CREATE UNIQUE INDEX IF NOT EXISTS magnum_idea_votes_user_idea_unique (user_id, idea_id)` + `magnum_idea_bookmarks_user_idea_unique` + `drizzle/schema.ts` `uniqueIndex` на оба `pgTable` + `journal idx 22` `when 1788300022000 tag 0022_idea_votes_unique` — закрывает `ON CONFLICT (user_id, idea_id) NeonDbError` при повторном `POST /magnum/api/ideas/:id/vote|bookmark` — `drizzle/migrations/0022_idea_votes_unique.sql` +4, `drizzle/migrations/meta/_journal.json` +7 idx22, `drizzle/schema.ts` +2
- 🗄️ **0023 — `magnum_gacha_history + magnum_gacha_free_rolls PK + board UNIQUE + flashmob 3`** — `51d2f05` `drizzle/migrations/0023_gacha_board_tx.sql` +21 `magnum_board_shares_user_day_unique` + `magnum_gacha_history (id, user_id, banner_type, rarity, cosmetic_id, is_new, created_at)` + `magnum_gacha_free_rolls (user_id, day_id) PK` + `idx_gacha_history_user` + `drizzle/schema.ts` +39 `magnumGachaHistory` + `magnumGachaFreeRolls PK` + `magnumFlashmobDays/Scores/Shares` 3 таблицы (`day PK / (user,day) PK`) + `journal idx 23` `when 1788300023000 tag 0023_gacha_board_tx` — **итого 30+ таблиц** (было ~28) — `drizzle/migrations/0023_gacha_board_tx.sql` +21, `drizzle/schema.ts` +39, `drizzle/migrations/meta/_journal.json` +7 idx23
- 📦 **server.ts imports — Pass42/Charts42/Flashmob42/Spin42 canon** — `51d2f05` `+4 imports` `XP_PER_LEVEL/MAX_LEVEL/SEASON_ID/PASS_REWARDS/xpForSource` из `src/lib/pass42.ts` + `TRACKS/seededSnapshots/isPeriod` из `src/lib/charts42.ts` + `FLASHMOB_TYPES/hashDayToSeed/getFlashmobForDay` из `src/lib/flashmob42.ts` + `SPIN_SECTORS/getStreakMultiplier` из `src/lib/spinRewards.ts` — подготовка Pass сезона + Charts + Flashmob дней + Spin колеса — `server.ts` +4

### ✨ Фичи — Spin Wheel 42 + Pass XP + Presave ref (делюкс инкремент)

- 🎡 **Daily Spin Wheel 42 — 8 секторов + стрик ×2/×3 + QR рефералка +1 спин обоим** — `51d2f05` **Neon** `magnum_daily_spin (user_id PK, last_spin, streak, free_spins, total_spins)` + `magnum_spin_referrals UNIQUE(inviter,invited)` + `ensureSpinTable` startup, **сектора 8** `SPIN_SECTORS` (dust 42/epic, skin, empty, spin +1), **стрик** `getStreakMultiplier(streak)` `1→×1 / 3→×2 / 7→×3`, **кулдаун 20ч** + `free_spins` bypass, **QR referral** `POST /magnum/api/spin/referral` код `42-XXXX` → `+1 free_spins` обоим + `ensureNotification «Колесо 42 — QR!»`, **скрипт** `src/lib/spinRewards.ts` `SPIN_SECTORS/getStreakMultiplier` — `server.ts` `ensureSpinTable/handleSpinStatus/handleSpin/handleSpinReferral` +~260
- 🎮 **Pass XP hooks — `addPassXp` в Eco + Mining** — `51d2f05` `handleEcoSubmit → addPassXp(user,20,'eco')` + `handleMiningClick → addPassXp(user,5,'mining')` — PASS сезон `SEASON_ID` из `pass42.ts` теперь капает XP с игр, без try-catch проброса — `server.ts` +2
- 🔗 **Presave ref UTM — `?ref=42-XXXX` + cookie `magnum_ref` auto-redeem** — `51d2f05` `handlePresaveClick` теперь `url 500` (было 300) + парсит `body.ref/referralCode` + `?ref=` из `body.url` + `req.url ?ref=` + `cookie magnum_ref` (`/^42-[A-Z0-9]{4}$/i`), при `refCode && invited_id empty` → `INSERT magnum_referrals` + `+42 coins` обоим + `transactions referral_in/referral_bonus via presave_ref` + selfCode guard `42-{userId base36}` — замыкает экономику рефералки через пресейв — `server.ts` `handlePresaveClick` +~40

### 🐛 Фиксы — TX + ROLLBACK + ON CONFLICT

- 🩹 **Gacha `ROLLBACK` на 402/429 — не виснет открытая TX** — `51d2f05` при `bal < price` теперь `ROLLBACK` перед `402 not enough coins`, при `dup free roll` `ROLLBACK` перед `429 already` — без утечки `idle in transaction` — `server.ts` +2
- 🩹 **DB error → 500 вместо краша bun** — оба TX обёрнуты `try { BEGIN … COMMIT } catch(e){ try ROLLBACK; console.error("[gacha roll/free-roll] tx failed",e); 500 db error }` — `magnum-bun` не падает на `NeonDbError` — `server.ts` +2
